...
export const calculateSOP = (terms, alphabet) => {
    ...
    result.SOPForHuman = SOPtoString(core, formatter);
    result.SOPForComputer = SOPtoString(core, wformatter);
    ...
    if (keys.length > 0){
        ...
        for (let i in solutions) {
            ...
            result.SOPAllForHuman.push(SOPtoString(SOP, formatter));
            result.SOPAllForComputer.push(SOPtoString(SOP, wformatter));
        }
        ...
        result.SOPBestForComputer = SOPtoString(core.concat(Array.from(solutions[min_route_key])), wformatter);
        result.SOPBestForHuman = SOPtoString(core.concat(Array.from(solutions[min_route_key])), formatter);
